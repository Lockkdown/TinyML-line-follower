...
def _sep_block(x: tf.Tensor, filters: int, l2_reg: float) -> tf.Tensor:
    """One SeparableConv2D block with BN and ReLU."""
...
def _conv_stem(inputs: tf.Tensor, l2_reg: float) -> tf.Tensor:
    ...
    reg = tf.keras.regularizers.l2(l2_reg)
    y = tf.keras.layers.Conv2D(16, 3, strides=2, padding="same", kernel_regularizer=reg)(inputs)
    y = tf.keras.layers.BatchNormalization()(y)
    return tf.keras.layers.ReLU()(y)


def _feature_trunk(x: tf.Tensor, l2_reg: float) -> tf.Tensor:
    """Stack of separable blocks and pools."""
    y = _sep_block(x, 32, l2_reg)
    y = tf.keras.layers.MaxPooling2D(2)(y)
    y = _sep_block(y, 64, l2_reg)
    y = tf.keras.layers.MaxPooling2D(2)(y)
    return _sep_block(y, 96, l2_reg)
...
def build_dsep_cnn(cfg: ModelConfig) -> tf.keras.Model:
    ...
    outputs = tf.keras.layers.Dense(cfg.num_classes, activation="softmax", kernel_regularizer=reg)(x)
